...
module.exports = {
    getAllProducts: async (req, res) => {
        try {
            ...
            const totalPage = Math.ceil(searchTotal.length / 5)
            return helper.response(res, 200, {
                totalData: searchTotal.length,
                activePage: (page / 5) + 1,
                totalPage: totalPage, searchResult
            });
        } catch (error) {
    ...
    updateProduct: async (req, res) => {
        try {
            const setData = {
                ...
                product_name: req.body.product_name,
                product_description: req.body.product_description,
                product_image: req.file === undefined ? '' : req.file.path,
                product_price: req.body.product_price,
            }
            const id = req.params.product_id
            let errorArr = []

            const { category_id, product_name, product_description, product_image, product_price } = setData;
            category_id === undefined || category_id === '' ? errorArr.push(1) : errorArr;
            product_name === undefined || product_name === '' ? errorArr.push(1) : errorArr;
            product_description === undefined || product_description === '' ? errorArr.push(1) : errorArr;
            product_price === undefined || product_price === '' ? errorArr.push(1) : errorArr;
            ...
            if (errorArr.length > 0) {
                ...
                const result = await updateProductTry1(setData, id);
                return helper.response(res,200, {message: 'Data Has Been Update'});
            }else{
                const result = await updateProductTry1(setData, id);
                return helper.response(res, 200, {message: 'Data Has Been Update'});
            }
